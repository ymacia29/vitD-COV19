...
DATA data.covid_s2;
	...
	LL92a LL91a LL85a TQ59a TQ69a TQ72a dh_vm_fq_itm11r dh_vm_fq_itm01r CO_Form
...
*FULL;
PROC FREQ data=data.covid_s2;
	WHERE CO_Form=93;
	TABLES evercovid symptomcovid severecovid hospitalcovid/ missing;
RUN;

PROC MEANS data=data.covid_s2;
	WHERE CO_Form=93;
	VAR CO_age CMsum meanPVI;
RUN;
PROC FREQ data=data.covid_s2;
	WHERE CO_Form=93;
	TABLES SE_RACE4 educ COsmoke COVID_PA CO_meno osteo obese 
	CVD lungdisease cancer diabetes AIdisease kidneyfail CMany 
	covid_household covidother indoorgather maskindoor_cat essential 
	patientcovid travel / missing NOCUM NOCOL NOPERCENT;
RUN;
